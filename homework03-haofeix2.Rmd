...
```{r}
library(tidyverse)
inspections <- read_csv("https://uofi.box.com/shared/static/lwwzlv2sulip1p8qoqqghmw0pouso0wk.csv")
head(inspections, 10)
```
...
```{r}
library(tidyverse)
jail_data$booking_date<-lubridate::mdy(jail_data$booking_date)
jail_data$released_date<-lubridate::mdy(jail_data$released_date)
str(jail_data)
```

```{r}
library(tidyverse)
inspections$`Inspection Date`<-lubridate::mdy(inspections$`Inspection Date`)
str(inspections)
```
...
inspections %>% 
  ...
  head(., 3)
```
The third most common facility type are Restaurant, Grocery Store, and School.
...
inmates <- jail_data
inmates <- inmates %>%
  mutate(booking_date_time = glue("{booking_date} {booking_time}")) %>%
  mutate(released_date_time = glue("{released_date} {released_time}")) %>%
  mutate(booking_date_time = as.POSIXct(booking_date_time)) %>%
  filter(released_date_time != "NA 00:00:00") %>%
  mutate(released_date_time = as.POSIXct(released_date_time)) %>%
  mutate(years = as.integer(difftime(released_date_time, booking_date_time, units = "days")) %/% 365) %>%
  mutate(days = as.integer(difftime(released_date_time, booking_date_time, units = "days") - years * 365)) %>%
  mutate(hours = as.integer(as.double(difftime(released_date_time, booking_date_time, units = "hours")) - years*365*24 - days*24)) %>%
  mutate(minutes = as.integer(as.double(difftime(released_date_time, booking_date_time, units = "mins")) - years*365*24*60 - days*24*60 - hours*60)) %>%
  mutate(seconds = as.integer(as.double(difftime(released_date_time, booking_date_time, units = "secs")) - years*365*24*60*60 - days*24*60*60 - hours*60*60 - minutes*60)) %>%
  mutate(time_spent_in_jail1 = as.double(years*365*24 + days*24 + hours + minutes/60 + seconds/60/60)) %>%
  mutate(time_spent_in_jail2 = as.double(difftime(released_date_time, booking_date_time, units = "hours"))) %>%
  rename_with(~sub('booking', 'booked', .))

inmates %>% 
  select(jacket_number, time_spent_in_jail1, time_spent_in_jail2) %>%
  head(., 10)
```
