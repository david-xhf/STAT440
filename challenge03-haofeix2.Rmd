...
```{r}
library(tidyverse)
covid = read.csv("https://github.com/nytimes/covid-19-data/raw/master/us-states.csv")
election = read.csv("https://uofi.box.com/shared/static/qmhzgom3e1y0lnqd720rnbhp34xyfd6d.csv")
tail(covid, 5)
tail(election, 5)
...
temp <- election %>%
   ...
   select(state, party_simplified, year)

result <- covid %>% 
   filter(date == '2020-11-02') %>%
   arrange(., desc(cases)) %>%
   mutate(state_upper = toupper(state)) %>%
   select(date, state, state_upper, cases, deaths) %>%
   head(10) %>%
   left_join(temp, by=c("state_upper" = "state")) %>%
   filter(year == 2016) %>% 
   select(date, state, cases, deaths, party_simplified, year)
```

REPUBLICAN is associated with Texas, DEMOCRAT is associated with California, REPUBLICAN is associated with Florida, DEMOCRAT is associated with New York, DEMOCRAT is associated with Illinois, REPUBLICAN is associated with Georgia, REPUBLICAN is associated with North Carolina, REPUBLICAN is associated with Tennessee, REPUBLICAN is associated with Arizona, and REPUBLICAN is associated with Wisconsin. 
...
```{r}
library(sqldf)
temp <- sqldf("SELECT state, party_simplified, year
       FROM election
       WHERE year == 2016
       GROUP BY state
       HAVING candidatevotes == max(candidatevotes)")

sqldf("SELECT date, covid.state, cases, deaths, party_simplified, year
       FROM covid JOIN temp ON temp.state = UPPER(covid.state)
...
```{r}
library(sqldf)
sqldf("SELECT date, temp.state, cases, deaths, party_simplified, year  
       FROM (
          ...
          ORDER BY cases DESC
          LIMIT 10
       ) AS temp LEFT JOIN election ON UPPER(temp.state) = election.state
       WHERE year == '2016'
       GROUP BY election.state
       HAVING candidatevotes == max(candidatevotes)
       ORDER BY cases DESC
      ")
```
