...
```{r}
library(tidyverse)
ads_demo <- read_csv("https://uofi.box.com/shared/static/9b9ecldtxkr23wb3uc36wwbn2l5ylpyx.csv")
```
```{r}
str(ads_demo)
...
```
```{r}
delivery_stores <- read_delim("https://uofi.box.com/shared/static/o2bqah52ioj0p0q4msvna7ixk8d9lsh4", delim=" ")
str(delivery_stores)
```
...
big_spender <- big_spender %>%
  ...
  select(-WEEK) %>%
  group_by(., PANID) %>%
  summarise(., total = sum(dollars999)) %>%
  filter(total > 15000)
big_spender
```
...
big_spender %>%
  ...
  select(PANID, total, Market_Name) %>%
  unique() %>%
  filter(!is.na(Market_Name))
```
...
```{r}
southern_grocery <- delivery_stores
southern_grocery <- southern_grocery %>%
  ...
  mutate(Market_Name = replace(Market_Name, Market_Name == "DALLAS, TX", "DALLAS")) %>%
  mutate(Market_Name = replace(Market_Name, Market_Name == "NEW ORLEANS, LA", "NEW ORLEANS")) %>%
  mutate(Market_Name = replace(Market_Name, Market_Name == "BIRMINGHAM/MONTG.", "BIRMINGHAM & MONTG.")) %>%
  mutate(Market_Name = replace(Market_Name, Market_Name == "RALEIGH/DURHAM", "RALEIGH & DURHAM")) %>%
  mutate(Market_Name = replace(Market_Name, Market_Name == "RICHMOND/NORFOLK", "RICHMOND & NORFOLK")) %>%
  arrange(Market_Name)
...
temp$`Panelist ID`
```
These panelists listed above spend at least one hundred dollars at each of at least 3 different grocery stores within the first 40 weeks of the year.
