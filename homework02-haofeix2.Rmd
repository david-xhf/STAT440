...
I felt like a little bit challenging when I was trying to format characters into dates. In those weeks, I constantly procrastinate, which made me unsuccessful in watching all videos and notes. However, successfully, I am able to watch them later and using some prior knowledge from other Statistics and Computer Science courses to figure out all homework.
...
**True**, .csv is also known as "comma-separated values file", which is a delimited text file that uses a comma to separate values.
...
- There was a peak of New Cases at the beginning of September;
- Fewest people got infected in a daily basis during the summer (May - July);
- There is an substantial increase of new cases beginning July.

**3 questions about the data**

- What are the trend of the data related to? Number of people who take the test solely or other factors like the spread of the virus.
- What does the gap between stratum mean?
- How can we predict the future trend of the data?
...
| Date | Average Daily New Cases (Approx.) |
| ------- | ------- |
| August, 2020 | 50 |
| September, 2020 | 100 |
| October, 2020 | 20 |
...
library(tidyverse)
library(rvest)
rental_data <- read_csv("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/rental-inspections-grades-data01.csv?token=AAABLVJRS7FRAJTGYRAM22DBLMQII")
## head(rental_data)
prop_url <- paste0("https://champaignil.devnetwedge.com/parcel/view/",rental_data$`Parcel Number`,"/2020")
TotalTax <- rep(0, length(prop_url))
for (i in 1:10){
  TotalTax[i] <- html_text(html_nodes(read_html(prop_url[i]), ".inner-value:nth-child(3)"), trim=TRUE)
}
head(TotalTax, 10)
```
...
```{r}
library(tidyverse)
rental_inspections <- read_delim("https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/rental-inspections-grades-data02.txt?token=AAABLVP3XOX3ZBYOS7I7BMTBLMT7W")
rental_inspections$`Inspection Date`<-lubridate::mdy(rental_inspections$`Inspection Date`)
rental_inspections$`Expiration Date`<-lubridate::mdy(rental_inspections$`Expiration Date`)
rental_inspections %>% 
  arrange(desc(Grade), `Inspection Date`) %>%
  head(., 10)
```
