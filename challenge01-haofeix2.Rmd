...
- [x] c. https://raw.github-dev.cs.illinois.edu/stat440-fa21/stat440-fa21-course-content/master/data/chicago-food-inspections-data.csv?token=AAABJG5E3NYYWDFJPNK77YDBG7QAM
...
**Markdown is a generic syntax while RMarkdown is an extension of Markdown, and RMarkdown is primarily used in RStudio, with embedded R codes which can be executed and knitted later to show all readers the same results.**
...
```{r}
library(tidyverse)
nuisance <- read_delim("https://data.urbanaillinois.us/api/views/tsn9-95m3/rows.tsv?accessType=DOWNLOAD&bom=true", 
    "\t")
str(nuisance)
```
...
```{R}
nuisance$`Date Reported`<-lubridate::mdy(nuisance$`Date Reported`)
# mode(nuisance$`Date Reported`)
# typeof(nuisance$`Date Reported`)
nuisance$`Date Notice Mailed or Given`<-lubridate::mdy(nuisance$`Date Notice Mailed or Given`)
# mode(nuisance$`Date Notice Mailed or Given`)
# typeof(nuisance$`Date Notice Mailed or Given`)
nuisance$`File Close Date`<-lubridate::mdy(nuisance$`File Close Date`)
# mode(nuisance$`File Close Date`)
# typeof(nuisance$`File Close Date`)
str(nuisance)
head(nuisance, 10)
```
