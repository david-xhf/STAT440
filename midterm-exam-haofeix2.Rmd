...
```{r}
library(tidyverse)
national <- read_csv("https://uofi.box.com/shared/static/c2whskv4x21w9pok3qxfh2kdz7jxk1g4.csv")
str(national)
```
...
```{r}
illinois <- read.csv("https://uofi.box.com/shared/static/ttc01y8x71eadmzc9ramd16ksuq2xckk.txt", header=FALSE)
colnames(illinois) <- c("state","sex","year","name","freq")
head(illinois, 5)
```
...
```{r}
illinois %>% 
  filter(., sex == "F" & name == "Xena") %>%
  arrange(desc(freq)) %>%
  mutate(rank=row_number()) %>%
  filter(rank == "1")
```
In 1996, "Xena" was the most frequently occurring female name in Illinois.
...
national %>% 
  arrange(name) %>%
  filter(name > "We[a-zA-Z]+" & name < "Zb[a-zA-Z]+") %>%
  sample_n(., 10)
```
...
```{r}
a_names <- illinois %>%
  filter(year == 2001 & str_detect(name, "A[a-zA-Z]+")) %>%
  mutate(len = str_length(name)) %>%
  arrange(freq, len, desc(name)) %>%
...
letters <- national %>% 
  filter(year == "2020")
counts <- tolower(unlist(str_extract_all(letters$name, "\\w")))
sort(table(counts), decreasing = TRUE)
```
Letter "a" is most commonly used among all names in the year of 2020.

**#14** Which sex's set of names has the largest total frequency? Use the imported dataset in **Problem 8** and a programming language applying data management concepts to answer the question. **Your answer to the question should be written using Markdown syntax, and you should include executable code with a printed result as evidence of your answer.**

```{r}
male <- illinois %>% filter(sex == "M")
male_total_freq = sum(male$freq)
female <- illinois %>% filter(sex == "F")
female_total_freq = sum(female$freq)
ifelse(male_total_freq > female_total_freq, "Male", "Female")
```
Male has the largest total frequency.
...
```{r}
length <- national %>% mutate(len = str_length(name))
quantile(length$len)
short = length %>% 
  filter(len <= 5) %>%
  nrow()
...
long
```
Long names are represented more in the data.
