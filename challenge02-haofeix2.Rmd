...
```{r}
library(tidyverse)
speed_dating <- read_csv("https://uofi.box.com/shared/static/w8vjq29rig8pdvussyg3997h1gwhkimq.csv")
tail(speed_dating, 10)
```
...
They are 238, 238, 249, 249, 237, 249, 241, 250, 250, 238; codes shown below.

```{r}
speed_dating %>%
  mutate(total = attr + sinc + intel + fun + amb + shar + like + prob) %>%
  arrange(desc(total)) %>%
  head(., 10) %>%
  select(iid, total)
```
...
24 interracial matches occur, however, we need to divided them by 2, since two people match each other, and only one match should be counted. So, the answer is **12**.

```{r}
speed_dating %>%
  count(samerace == 0 & match == 1)
```
